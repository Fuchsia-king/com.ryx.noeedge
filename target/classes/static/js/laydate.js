...
; !function () {
    ...
    */
    ...
    let initCalendar = true;
    //日历表
    Class.prototype.calendar = function (value, isInit = true) {
        var that = this
            , options = that.config
        ...
        //赋值双日历
        if (options.range && !value && isInit) {
            let value = document.getElementById("dadyInput").value;
            if (value) {
                const defalutData = {
                    year: 2020,
                    month: 5,
                ...
                let secondsDate = value.split(" 到 ")[1].split("-");
                let [year, month] = secondsDate;
                month = month - 2;
                Object.assign(defalutData, { year, month });
                var EYM = that.getAsYM(defalutData.year, defalutData.month);
                that.calendar(lay.extend({}, defalutData, {
                    year: EYM[0]
                    , month: EYM[1]
                }));
            } else {
    ...
    Class.prototype.change = function (index) {
        ...
        return {
            prevYear: function (index) {
                if (addSubYeay('sub')) return;
                let calendarValue;
                if (index) {
                    calendarValue = that.computedDateTime(false, "year");
                } else {
                    dateTime.year--;
                }
                that.checkDate('limit').calendar(calendarValue, false);
                options.range || that.done(calendarValue, 'change');
            }
            , prevMonth: function (index) {
                let calendarValue;
                if (index) {
                    // 计算出下一年的数据，calendar()更新日历
                    calendarValue = that.computedDateTime(false, "month");
                } else {
            ...
            , nextMonth: function (index) {
                ...
                if (index) {
                    // 计算出下一年的数据，calendar()更新日历
                    calendarValue = that.computedDateTime(true, "month");
                } else {
            ...
            , nextYear: function (index) {
                ...
                if (index) {
                    // 计算出下一年的数据，calendar()更新日历
                    calendarValue = that.computedDateTime(true, "year");
                } else {
                    dateTime.year++
                }
                that.checkDate('limit').calendar(calendarValue, false);
    ...
    /*
    @params:{
       way:true 为加法 false 为减法
    }
    */
    Class.prototype.computedDateTime = function (way, type) {
        const defalutData = {
            year: 2020,
            month: 5,
            date: 0,
        ...
        let elDateTime = document.querySelectorAll("[lay-type='year']")[1].getAttribute('lay-ym');
        let [year, month] = String(elDateTime).split("-");
        switch (type) {
            case "year":
                if (way) {
                    year = Number(year) + 1;
        ...
        Object.assign(defalutData, { year, month });
        let calendarDateTime = defalutData;
        var EYM = this.getAsYM(calendarDateTime.year, calendarDateTime.month);
        calendarDateTime.year = EYM[0],
            calendarDateTime.month = EYM[1]
        return calendarDateTime;
        // return EYM;
        // return lay.extend({}, calendarDateTime, {
        //     year: EYM[0]
        //     , month: EYM[1]
        // })
    },
        ...
        Class.prototype.changeEvent = function () {
            ...
            lay.each(that.elemHeader, function (i, header) {
                ...
                lay(header[0]).on('click', function (e) {
                    that.change(i).prevYear(i);
                });

                //上一月
                lay(header[1]).on('click', function (e) {
                    that.change(i).prevMonth(i);
                });
                ...
                lay(header[3]).on('click', function (e) {
                    that.change(i).nextMonth(i);
                });

                //下一年
                lay(header[4]).on('click', function (e) {
                    that.change(i).nextYear(i);
                });
            });
